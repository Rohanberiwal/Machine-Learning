...
def train_double_dqn(env, max_episodes=1000, batch_size=64, gamma=0.99, lr=1e-4, target_update_tau=0.005, max_grad_norm=10, alpha=0.6, beta_start=0.4, early_stop_patience=30, reward_threshold=None):
    ...
    for episode in tqdm(range(max_episodes)):
        state = env.reset()
        total_reward, done = 0, False
        beta = min(1.0, beta_start + episode * (1.0 - beta_start) / max_episodes)
        ...
        all_f1s.append(f1)
        if episode == max_episodes - 1 or early_stop_counter >= early_stop_patience:
            final_preds = episode_preds
            final_targets = episode_targets
            final_probs = episode_probs
        if reward_threshold is not None and total_reward >= reward_threshold:
            break
