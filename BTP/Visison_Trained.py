...
def prepare_labels(image_paths, label_dict):
    ...
    for path in image_paths:
        labels.append(label_dict.get(path, -1))  # Default to -1 if the path is not found in label_dict
    return torch.tensor(labels, dtype=torch.long)

def fine_tune_vit(image_folder, model, label_dict, epochs=1, batch_size=16, learning_rate=5e-5):
    images, image_paths = load_images_from_folder(image_folder)
    ...
    model.train()
    loss_list = []  # Initialize the loss_list to track the loss during training
    ...
    for epoch in range(epochs):
        ...
        loss_list.append(loss.item())  # Append loss for each epoch

        print(f"Epoch {epoch+1}/{epochs}, Loss: {loss.item()}")

    plt.plot(range(epochs), loss_list, label='Training Loss')  # Plot the loss
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
...
fine_tune_vit(image_folder, model, labels_dict)
print("Code ends")
...
def extract_zip(zip_path, extract_to):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            zip_ref.extractall(extract_to)
        print(f"Extraction successful! Files are extracted to {extract_to}")
    except Exception as e:
...
def process_images(image_folder, model, feature_extractor):
    predictions = {}
    for filename in os.listdir(image_folder):
        img_path = os.path.join(image_folder, filename)
        try:
            image = load_image(img_path)
            prediction = predict_image(image, model, feature_extractor)
            predictions[img_path] = prediction
            print(f"Processed {filename}, Prediction: {prediction}")
        except Exception as e:
            print(f"Error processing {filename}: {e}")
    return predictions

zip_path = '/content/BTP.zip'
extract_to = '/content/Testerfile'
image_folder = '/content/Testerfile/Histopathology images'
extract_zip(zip_path, extract_to)
predictions = process_images(image_folder, model, feature_extractor)
print(predictions)
