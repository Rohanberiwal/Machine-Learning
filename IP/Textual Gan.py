...
for epoch in range(num_epochs):
    ...
    for real_data in dataloader:
        ...
        fake_data = generator(z).argmax(dim=2)
    ...
    print(f"Epoch [{epoch+1}/{num_epochs}], Loss D: {epoch_d_loss / len(dataloader):.4f}, Loss G: {epoch_g_loss / len(dataloader):.4f}")

z = torch.randn(1, latent_dim)  # Sample random noise
generated_data = generator(z)
generated_substitute = generated_data.argmax(dim=2)
